...
$resultado_banco = $con->query($sql);

if (!$resultado_banco || $resultado_banco->num_rows === 0) {
    die("Nenhuma atividade localizada para a geração do relatório global.");
}
...
<head>
    <style>
        ...
        .page-delimiter { page-break-after: always; }
    </style>
</head>
<body>';

$total_registros = $resultado_banco->num_rows;
$contador = 0;


while ($atv = $resultado_banco->fetch_assoc()) {
    $contador++;
    ...
    <div class="banner-title-container">
        <div class="main-title">Relatório de Evento Institutional</div>
        <div class="sub-title">Consolidado Geral de Atividades (' . $contador . '/' . $total_registros . ')</div>
    </div>

    <div class="section-header">Dados Rápidos da Ação</div>
    ...
    <table class="signature-container">
        <tr>
            <td width="45%" class="signature-box">
                <strong>Prof. ' . htmlspecialchars($usuario_nome) . '</strong><br>
    ...
    <div class="footer-banner">
        ...
        <span style="font-weight: bold; color: #003366; font-size: 8px; margin-top: 4px;">SisMAB • Sistema de Monitoramento de Atividades da Biblioteca</span>
    </div>';
...
$arquivo = "Relatorio_Consolidado_SisMAB_" . date('Ymd_His') . ".pdf";
...
$mpdf->setFooter('FIRJAN SENAI SAPUCAÍ | Relatório Consolidado Geral | Página {PAGENO} de {nbpg}');
$mpdf->WriteHTML($html); 
